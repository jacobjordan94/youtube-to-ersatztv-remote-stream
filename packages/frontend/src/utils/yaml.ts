...
export const formatDescription = (description: string, format: 'string' | 'folded' | 'literal'): string => {
  switch (format) {
    case 'string':
...
export const generateYaml = (
  ...
  if (metadata.isLive) {
    ...
    duration = settings.livestreamDuration === 'custom' ? settings.customLivestreamDuration : settings.livestreamDuration;
  } else {
    ...
    switch (settings.durationMode) {
      ...
      case 'api-padded':
        ...
        const paddedMinutes = Math.ceil(totalMinutes / settings.paddingInterval) * settings.paddingInterval;
        const paddedHours = Math.floor(paddedMinutes / 60);
        const paddedMins = paddedMinutes % 60;
        duration = [paddedHours, paddedMins, 0]
